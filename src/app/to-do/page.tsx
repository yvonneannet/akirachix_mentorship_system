  ...
  return (
    <Layout > 
      <div className="max-w-4xl mx-auto p-4">
        <h1 className="text-2xl font-bold mb-6 mt-12 text-center">To-Do List</h1>
        <div className="mb-4 flex">
          <input
            type="text"
            value={newTask}
            onChange={(e) => setNewTask(e.target.value)}
            className="flex-grow border border-r-0 rounded-l px-4 py-2"
            placeholder="Add new task"
          />
          <button onClick={addTask} className="bg-green-500 text-white px-4 py-2 rounded-r border border-green-500">
            Add Task
          </button>
        </div>
        <ul>
          {tasks.map(task => (
            <li key={task.id} className="mb-2 p-3 bg-white shadow rounded-lg flex items-center">
              <input
                type="checkbox"
              ...
              {editingTaskId === task.id ? (
                <button onClick={() => saveTaskEdit(task.id)} className="text-green-500 mr-2">
                  <Save size={18} />
                </button>
              ) : (
                <button onClick={() => startEditingTask(task.id, task.text)} className="text-blue-500 mr-2">
                  <Edit size={18} />
                </button>
              )}
              <AlertDialog.Root>
